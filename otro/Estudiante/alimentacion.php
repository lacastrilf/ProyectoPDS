...
$sql = "SELECT SUM(gasto) AS total_gastos FROM gastosI WHERE id_usuario='$idUsuario' AND tipo='Alimentacion'";
$resultado = $conexion->query($sql);
$totalGastos = 0;
if ($resultado->num_rows > 0) {
    $dato = $resultado->fetch_assoc();
...
if ($totalGastos > $presupuesto) {
    $sqlNotificacion = "SELECT * FROM notificaciones WHERE id_usuario='$idUsuario' AND tipo='Alimentacion'";
    $resultadoNotificacion = $conexion->query($sqlNotificacion);

    if ($resultadoNotificacion->num_rows == 0) {
        $mensaje = "Has excedido tus gastos de alimentación esta semana.";
        $titulo = "Presupuesto superado";
        $fecha = date('Y-m-d H:i:s'); // Obtener la fecha y hora actual
        $sqlInsertNotificacion = "INSERT INTO notificaciones (id_usuario, tipo, icono, color, titulo, mensaje, fecha) VALUES ('$idUsuario', 'Alimentacion', 'exclamation-circle', 'warning', '$titulo', '$mensaje', '$fecha')";
...
// Si los gastos son menores o iguales al presupuesto, eliminar cualquier notificación existente
    ...
    if ($resultadoNotificacion->num_rows > 0) {
        $sqlDeleteNotificacion = "DELETE FROM notificaciones WHERE id_usuario='$idUsuario' AND tipo='Alimentacion'";
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Alimentacion</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
          ...
          <div class="row">
            ...
            <div class="col-12">
              <div class="card">
                ...
                <div class="card-body">
                  <h5 class="card-title">Grafica Gasto<span>/Semanal</span></h5>
            ...
            <div class="card-body">
              ...
              <div class="activity">
                <div style="height: 390px;">
                  <?php
                  $sqlGetEventos = "SELECT * FROM gastosi WHERE id_usuario='$idUsuario' AND tipo='Alimentacion'";
      ...
      $(document).ready(function() {
          ...
          function checkMax(input) {
              if (input.value > parseInt(input.getAttribute('max'))) {
                  alert("El valor no puede ser mayor que el máximo permitido");
